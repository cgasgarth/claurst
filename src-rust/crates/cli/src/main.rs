...
async fn main() -> anyhow::Result<()> {
    ...
    let client_config = claurst_api::client::ClientConfig {
        ...
        use_bearer_auth,
        provider: if config.provider.as_deref() == Some("openai-codex") {
            claurst_api::client::Provider::Codex
        } else {
            claurst_api::client::Provider::Anthropic
        },
        ..Default::default()
    };
    let client = Arc::new(
...
fn spawn_models_cache_refresh() {
    ...
    tokio::spawn(async move {
        ...
        if let Ok(resp) = client
            .get(&url)
            .header("User-Agent", "Claurst/0.0.10")
            .send()
            .await
        {
